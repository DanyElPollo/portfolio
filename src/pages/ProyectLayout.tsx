    ...
    <>
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5">
        {proyectos && proyectos?.map((p) => (
          <CardBase key={p.id} id={p.id} title={p.name} >
            <div className="flex w-auto justify-end">
              ...
              <Href href={p.html_url} className="mx-2 inline-flex items-center justify-center p-2 rounded-full transition-all duration-300 ease-in-out bg-black hover:scale-125 hover:bg-white hover:text-black" _target="_blank"
              >
